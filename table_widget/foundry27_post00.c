...
int cb_divider_drag00(PtWidget_t *divider, void *d, PtCallbackInfo_t *cbinfo) {
  ...
  while ((row = PtWidgetBrotherBehind(row)) != NULL) {
    ...
    /* this does nothing :( */
    PtSetResource(tmp, Pt_ARG_MIN_WIDTH,
        dvdr_cb->sizes[pos].to - dvdr_cb->sizes[pos -1].to, 0);
    PtSetResource(tmp, Pt_ARG_WIDTH,
        dvdr_cb->sizes[pos].to - dvdr_cb->sizes[pos -1].to, 0);
  }
...
int main( int argc, char *argv[] ) {
  ...
  /* wtf? why does PtText get so narrow that no character is to be seen? */
  ...
  /* this is ignored by PtDivider */
  PtSetArg( &args[i++], Pt_ARG_MIN_WIDTH, 70, 0);
  /* because PtDivider ignores MIN_WIDTH, this is necessary to set the initial
     width */
